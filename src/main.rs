...
async fn main() -> Result<()> {
    ...
    let (tx_sequencer_left, rx_sequencer_left) = mpsc::channel(1);
    let (tx_sequencer_right, rx_sequencer_right) = mpsc::channel(1);
    let (tx_mixer, rx_mixer) = mpsc::channel(1);
    ...
    let mut sequencer_a =
        EuclideanSequencer::new(rx_sequencer_left, tx_gui, tx_sequence.clone(), shared_state);
